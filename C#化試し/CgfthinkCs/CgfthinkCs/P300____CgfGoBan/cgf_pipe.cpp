...
int IsGtpEngineExist(void)
{
	...
	string p = strstr(str," ");	// "Program Files"で失敗する
	if ( p != NULL ) *p = 0;
...
// 座標を'K15'のような文字列に変換
void change_z_str(string str, int z)
{
	int x,y;
...
// GnuGoの初期化
int init_gnugo_gtp(string sName)
{
...
static const string status_names[6] =
